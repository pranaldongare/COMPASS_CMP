...
async def create_export(
    ...
    *,
    ...
    #: Each destination, its country and the ground the transfer rested on (S2-04).
    ...
    row = await fetch_one(
        ...
        INSERT INTO export_log (project_id, site_id, export_type, exported_by,
                                row_count, file_hash, file_ref, transfer_basis)
        VALUES (%s, %s, %s::export_type, %s, %s, %s, %s, %s)
        RETURNING export_id, export_uuid, export_type, exported_at, row_count, file_hash,
                  file_ref, transfer_basis
        """,
        (
            project_id,
            site_id,
            ...
            file_hash,
            file_ref,
            Jsonb(transfer_basis) if transfer_basis is not None else None,
        ),
    )
...
async def add_export_lines(
    conn: Conn, export_id: int, lines: list[tuple[int, int, int | None, str | None]]
) -> int:
    """One row per subject in the file. This is the disclosure record.

    Each line: the person, the consent, and where the row went - the processor
    running its site and that processor's country, as they were at the export.
    """
    if not lines:
        return 0
    values = ", ".join(["(%s, %s, %s, %s, %s)"] * len(lines))
    params: list[Any] = []
    for user_id, consent_id, processor_id, country in lines:
        params.extend([export_id, user_id, consent_id, processor_id, country])
    cur = await conn.execute(
        f"""INSERT INTO export_line (export_id, auth_user_id, consent_id,
    ...
    **Consented, partial and withdrawn - not declined.** An agent working from a
    ...
    return await fetch_all(
        ...
        SELECT u.id AS auth_user_id, u.uuid AS subject_uuid, u.full_name, u.email,
               ...
               s.processor_id AS destination_processor_id,
               dp.legal_name AS destination_name, dp.location_country AS destination_country,
               ...
               array_agg(p.purpose_code ORDER BY p.purpose_code)
                 FILTER (WHERE g.granted AND NOT p.cross_border_permitted)
                 AS granted_not_cross_border
          ...
          LEFT JOIN processor dp ON dp.processor_id = s.processor_id
...
_RESTRICTED = """
  rc.country_id, rc.country_uuid, rc.country_code, rc.notification_ref, rc.listed_at, rc.lifted_at,
  lb.full_name AS listed_by_name, fb.full_name AS lifted_by_name
  FROM restricted_country rc
  JOIN auth_user lb      ON lb.id = rc.listed_by
  LEFT JOIN auth_user fb ON fb.id = rc.lifted_by
"""


async def restricted_active(conn: Conn, country_code: str) -> Row | None:
    """The listing that restricts this country now, if there is one."""
    return await fetch_one(
        conn,
        f"SELECT {_RESTRICTED} WHERE rc.country_code = %s AND rc.lifted_at IS NULL",
        (country_code,),
    )


async def restricted_list(conn: Conn, *, active_only: bool) -> list[Row]:
    where = "WHERE rc.lifted_at IS NULL" if active_only else ""
    return await fetch_all(
        conn, f"SELECT {_RESTRICTED} {where} ORDER BY rc.country_code, rc.listed_at DESC"
    )


async def restricted_by_uuid(conn: Conn, country_uuid: str) -> Row | None:
    return await fetch_one(
        conn, f"SELECT {_RESTRICTED} WHERE rc.country_uuid = %s", (country_uuid,)
    )


async def restrict_country(
    conn: Conn, *, country_code: str, notification_ref: str, listed_by: int
) -> Row:
    row = await fetch_one(
        conn,
        """INSERT INTO restricted_country (country_code, notification_ref, listed_by)
           VALUES (%s, %s, %s) RETURNING country_uuid""",
        (country_code, notification_ref, listed_by),
    )
    assert row is not None
    listed = await restricted_by_uuid(conn, str(row["country_uuid"]))
    assert listed is not None
    return listed


async def lift_restriction(conn: Conn, country_uuid: str, *, lifted_by: int) -> None:
    await conn.execute(
        """UPDATE restricted_country SET lifted_at = clock_timestamp(), lifted_by = %s
            WHERE country_uuid = %s AND lifted_at IS NULL""",
        (lifted_by, country_uuid),
    )
