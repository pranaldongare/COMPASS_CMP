...
async def place(
    ...
    *,
    ...
    # Items already applied and waiting on a store record the hold now, rather
    # than at the next sweep - the scope card shows it the moment it is placed.
    await _run_covered(conn, asset_id=asset_id, subject_user_id=subject_id, actor_id=actor_id)
    return held
...
async def release(conn: Conn, *, hold_uuid: str, actor_id: int) -> Row:
    ...
    await _run_covered(
        conn, asset_id=row["asset_id"], subject_user_id=row["subject_user_id"], actor_id=actor_id
    )
...
async def _run_covered(
    conn: Conn, *, asset_id: int | None, subject_user_id: int | None, actor_id: int
) -> None:
    """Run the executor over every applied, unfinished item a hold covers."""
    for item in await rights_repo.items_awaiting_execution(conn):
        if asset_id is not None and int(item["asset_id"]) != int(asset_id):
            continue
        request = await rights_repo.by_id(conn, int(item["request_id"]))
        if not request:
            continue
        if subject_user_id is not None and request.get("subject_user_id") != int(subject_user_id):
            continue
