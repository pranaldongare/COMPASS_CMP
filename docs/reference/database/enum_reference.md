...
[Guide](README.md) · [Enum SVG](enums.svg)

All 39 enum types, in PostgreSQL enum ordering within each type.
...
| export_type | `collection_pack`, `consented_list`, `project_export` |
| language_code | `english`, `hindi`, `marathi`, `tamil`, `telugu`, `kannada`, `bengali`, `gujarati` |
...
| notice_audience | `data_subject`, `employee`, `ex_employee`, `others` |
| notice_status | `draft`, `approved`, `published`, `superseded` |
| person_type | `external`, `employee`, `ex_employee`, `vendor` |
| processor_request_status | `pending`, `approved`, `rejected` |
| processor_type | `lab`, `tool`, `other` |
| project_status | `in_draft`, `under_process`, `pending_approval`, `approved`, `closed` |
...
| rights_holder_source | `export_line`, `asset_consent`, `manual` |
| rights_item_state | `proposed`, `decided`, `instructed`, `applied` |
...
| rights_verification_method | `session`, `code`, `manual` |
| rights_verification_status | `pending`, `verified`, `failed` |
