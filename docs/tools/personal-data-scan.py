...
* `backend/api/openapi.json` - every operation and every field of every request
  ...
  deep is still found;
* `docs/reference/access-control/endpoint_permissions.json` - the guard on each route and
  the permission matrix's answer per role;
* `docs/reference/database/schema_inventory.json` - every table and column, used only to
...
the document stays true.
    ...
    python3 docs/tools/personal-data-scan.py            # the tables
    python3 docs/tools/personal-data-scan.py --check    # only the warnings
"""
...
def load() -> tuple[dict, list, dict]:
    ...
    perms = json.loads((ROOT / "docs/reference/access-control/endpoint_permissions.json").read_text())
    schema = json.loads((ROOT / "docs/reference/database/schema_inventory.json").read_text())
