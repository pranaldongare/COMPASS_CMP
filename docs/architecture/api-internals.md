...
A layered FastAPI service over PostgreSQL 16 with **no ORM**, serving 249
endpoints across 34 tables. The repository-wide picture, including the two
...
**Personal data is sealed at rest, and served sealed.** A repository write
passes the row through `seal()` in `infrastructure/dkms/`, which encrypts its
personal columns in one call to the key service. The API serves those columns
as stored, `SE::…`, and the portal's server opens them. The API opens a value
only where it must act on it - send a code, write a CSV, compare what was
typed - and never hands the result to a response. A row is found by a sealed
value through its keyed hash, never by the value. See
[ADR 0016](../decisions/0016-personal-data-sealed-by-a-separate-key-service.md)
and [docs/dkms/](../dkms/README.md).
...
Four things were added after this overview was first written, and each has
its own page: the collection model and site ownership
([docs/domain/collection-and-routing.md](../domain/collection-and-routing.md)),
the rights module ([rights-module.md](rights-module.md)), mobile-first identity with a
second factor for every staff role
([authentication.md](../security/authentication.md)), and personal data sealed
by a separate key service and looked up by keyed hash
([docs/dkms/](../dkms/README.md),
[encryption-at-rest.md](../security/encryption-at-rest.md)). The decisions behind
