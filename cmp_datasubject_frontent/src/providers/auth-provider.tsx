...
export function RequireAuth({
  ...
  React.useEffect(() => {
    ...
    // A partial staff session - password accepted, second factor outstanding -
    // can only be completed on the staff console. This portal has no verify
    // step, because nobody who belongs here has a password.
    if (needsMfa) {
      // An absolute URL on another origin, so `router` is the wrong tool.
      const verify = new URL("/sign-in/verify", config.staffPortalUrl);
  ...
  // The session cookie is shared with the staff console when both run on the
  // same host, so a DPO who signed in there and opened this portal arrives
  // signed in. Nothing here is for her; say where she belongs.
